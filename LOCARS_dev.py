...
class PrinterControlPage(QWidget):
    def __init__(
        ...
        # Connect joystick button to toggle joystick control
        self.ui.joystick_button.toggled.connect(self.toggle_joystick)
        ...
        if checked:
            ...
            if self.joystick_thread is None or not self.joystick_thread.is_alive():
                self.joystick_thread = threading.Thread(
                    target=self.joystick_control, daemon=True
                )
                self.joystick_thread.start()
        else:
            print("🖥️ UI control enabled")

    def joystick_control(self):
        """Continuously read joystick input and move the printer."""
        while self.joystick_enabled and self.running:
            pygame.event.pump()  # Process events

            if self.joystick:
                x_axis = self.joystick.get_axis(0)  # Left/Right
                y_axis = self.joystick.get_axis(1)  # Forward/Backward
                z_axis = self.joystick.get_axis(3)  # Rotation or Z-axis if available

                threshold = 0.1  # Ignore minor movements
                speed = 2000  # Default speed

                if abs(x_axis) > threshold:
                    self.send_gcode_once("X", round(x_axis * 5, 2))  # Scale movement
                if abs(y_axis) > threshold:
                    self.send_gcode_once("Y", round(-y_axis * 5, 2))  # Invert Y
                if abs(z_axis) > threshold:
                    self.send_gcode_once("Z", round(z_axis * 2, 2))  # Z movement
    ...
    def send_gcode_once(self, direction, increment):
        ...
        if self.joystick_enabled:
            print(f"🎮 Joystick Moving {direction} by {increment}")
        else:
            print(f"🖥️ UI Moving {direction} by {increment}")
    ...
    def start_continuous_move(self, direction, increment):
        """Start moving continuously in a given direction if UI control is active."""
        if self.joystick_enabled:
            return
...
class AcquisitionPage(QWidget):
    ...
    def update_followup_grid(self, all_wells, active_wells):
        try:
            ...
            # ✅ Retrieve rows & cols directly from ChipPage
                ...
                for col in range(cols):
                    ...
                    if well_label in all_wells:
                        ...
                        if well_label not in checked_wells:
                            checkbox.setEnabled(
                                False
                            )  # Disable if it wasn't checked in ChipPage
    ...
    def move_to_wells_and_home(self):
        ...
        try:
            ...
            for index, (name, (x, y)) in enumerate(active_wells):
                ...
                try:
                    send_gcode(self.printer, "G1 Z3 F500")  # Lift before move
                    send_gcode(self.printer, gcode_command_xy)  # Move to XY position
                    time.sleep(0.5)  # ✅ Allow time for movement
                    send_gcode(self.printer, "G1 Z-3 F500")  # Lower for acquisition
                    time.sleep(0.5)  # ✅ Allow time for acquisition

                    # ✅ Check the checkbox *after* reaching the well
                    for checkbox in self.checkboxes:
                        if checkbox.text() == name:
                            self.checkbox.setChecked(True)
                            break  # ✅ Stop after checking the correct checkbox

                    time.sleep(1)  # ✅ Simulate acquisition duration at the well

                    send_gcode(self.printer, "G1 Z3 F500")  # Move back up
                    time.sleep(0.5)  # ✅ Allow time before next move

                except pyvisa.errors.InvalidSession:
                    print("❌ Session invalid. Ensure printer is properly connected.")
                    return  # 🔥 No reconnection attempt, just exit
...
lumed_ips_path = r"C:\Users\tommy\Desktop\New_LOCARS\src\LOCARS\lumed_ips\src"
...
class LOCARS(QMainWindow):
    def __init__(self):
        ...
        print("✅ LOCARS initialized successfully.")
    ...
    def closeEvent(self, event):
        # Close the log window first to release the file safely
